<template>
  <view class="container">
    ...
    <view class="ranking-list">
      ...
      <view v-else-if="friendRankings.length === 0" class="empty-list">
        <text>暂无好友排行数据</text>
      </view>

      <view v-else class="ranking-items">
        <view v-for="(item, index) in friendRankings" :key="item.user_id" class="ranking-item">
          <view class="rank-number" :class="{ 'top-three': index < 3 }">{{ item.rank || (index + 1) }}</view>
      ...
      <view class="pagination" v-if="totalPages > 1">
        <view class="page-btn" :class="{ disabled: currentPage <= 1 }"
          @click="currentPage > 1 && changePage(currentPage - 1)">上一页</view>
        <text class="page-info">{{ currentPage }}/{{ totalPages }}</text>
        <view class="page-btn" :class="{ disabled: currentPage >= totalPages }"
          @click="currentPage < totalPages && changePage(currentPage + 1)">下一页</view>
      </view>
    </view>
    
    <!-- 添加好友按钮 -->
    <view class="friend-btn" @click="goToFriendPage">添加好友</view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    changeTimeRange(range) {
      ...
      this.currentPage = 1; // 切换时间范围时重置页码
      this.loadRankingData();
    },
